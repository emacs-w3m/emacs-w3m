...
AC_CACHE_VAL(EMACS_cv_SYS_$1,[
	...
	if test -z "${XEMACSDEBUG}"; then
	echo '';
	echo '::::::: shall command :::::::';
	echo "${EMACS} ${VANILLA_FLAG} -batch -l ${EL} >& AC_FD_CC"
dnl		"${EMACS}" ${VANILLA_FLAG} -batch -l ${EL} >& AC_FD_CC 2>&1
		"${EMACS}" ${VANILLA_FLAG} -batch -l ${EL} >& AC_FD_CC
	else
		"${XEMACSDEBUG}" "${EMACS}" ${VANILLA_FLAG} -batch -l ${EL} >& AC_FD_CC 2>&1
	fi
	retval="`cat ${OUTPUT}`"
