...
;; Copyright (C) 2003-2011, 2013-2019, 2021-2026 Katsumi Yamaoka
...
You should set `w3m-use-cookies' and `w3m-use-form' to non-nil"))
  ...
  (if (not (and w3m-use-cookies w3m-use-form name password))
    ...
    (w3m-cookie-setup)
    (let ((case-fold-search t) tem)
      (dolist (cookie w3m-cookies)
	(and (setq tem (w3m-cookie-domain cookie))
	     (string-match "\\`sankei\\.\\|\\.sankei\\." tem)
	     (setq tem (w3m-cookie-url cookie))
	     (string-match "[^/]*login[^/]*\\'" tem)
